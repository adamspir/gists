...
match (a:ScheduledTask{name:'insurance_cli_old'}), (b:Service {onoma:'kumquat'})
create (a)-[:SEND_TO]->(b);
...
=== What if KumquatDB fails. Which systems are affected?
//hide
[source,cypher]
----
MATCH (n:Database{onoma:"kumquat"})-[r]->(m) MATCH (m)<-[]-(f)
return n,m,f;
----
//graph_result
